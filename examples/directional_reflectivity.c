...
int main(void) {
    ...
    const float lambda_vac = lambda_list[46];

    const double complex eta_dat = n_dat[46] + k_dat[46] * I;

    double complex n_list[] = {
        1.0 + 0.0 * I, eta_dat, 1.5 + 0.0 * I
    };

    const double d_list[] = {INFINITY, 0.300, INFINITY};  // units of micron

    const uint8_t num_orientations = 80;

    double th_list[num_orientations];

    for (int i = 0; i < num_orientations; i++)
    {
        const uint8_t deg = i;  // units of degrees
        th_list[i] = deg * (PI / 180.0);  // units of rad
    }

    double R_list[num_orientations];
    double T_list[num_orientations];
    ...
    for (int i = 0; i < num_orientations; i++)
    {
        coh_tmm(
            ...
            th_list[i],  // const double th_0,
            lambda_vac,  // const double lam_vac,
